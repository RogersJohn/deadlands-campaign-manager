...
2. **SessionsPage.js**
   - `createSession(name, description, maxPlayers)`
   - `joinSession(sessionName, characterName)`

3. **SessionRoomPage.js**
   - `isSessionRoomLoaded()`
   - `clickStartGame()`
   - `getPlayerCount()`
   - `waitForPlayerToJoin(username, timeout)`
